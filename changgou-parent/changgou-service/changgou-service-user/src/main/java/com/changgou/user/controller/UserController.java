 ...
 */
    ...
    public Result incrUserPoints(Integer point) {
        ...
        userService.incrUserPoints(username, point);
        return new Result(true, StatusCode.OK, "积分新增成功");
    }
/**
*用户登录
 * @param username :
 ...
 */
    ...
    public Result login(String username, String password) {
        ...
        if (StringUtils.isEmpty(username) || StringUtils.isEmpty(password)) {
            return new Result(false, StatusCode.LOGINERROR, "登陆失败,请输入用户名或密码");
        }
        //授权
        User user = userService.findById(username);
        if (user == null || user.getUsername() == null) {
            return new Result(false, StatusCode.LOGINERROR, "登陆失败,该用户不存在");
        }
        //判断密码是否正确
        if (BCrypt.checkpw(password, user.getPassword())) {
            //登录成功签发jwt令牌
            Map<String, Object> jwtInfo = new HashMap<>();
            jwtInfo.put("role", "USER");
            jwtInfo.put("success", "SUCCESS");
            jwtInfo.put("username", user.getUsername());
            String jwt = JwtUtil.createJWT(UUID.randomUUID().toString(), JSONObject.toJSONString(jwtInfo), null);
            return new Result(true, StatusCode.OK, "登录成功", jwt);
        }
        return new Result(false, StatusCode.LOGINERROR, "登陆失败,用户名或密码错误");
    }
